...
def main():
  ...
  for chain in itertools.permutations(['a' + str(x) for x in range(0,domainSize)]):
    for length in range(1, len(chain)):
      ...
      rule1 = 'pol1(' + delegation[-1] + ') :- owner1TF(' + chain[0] + ')'
      rule2 = 'pol2(' + delegation[-1] + ') :- owner2TF(' + chain[0] + ')'
      for cur in range(0, len(delegation)-1):
        rule1 += ' ^ valid1(' + delegation[cur] + ',' + delegation[cur+1] + ')'
        rule2 += ' ^ valid2(' + delegation[cur] + ',' + delegation[cur+1] + ')'
